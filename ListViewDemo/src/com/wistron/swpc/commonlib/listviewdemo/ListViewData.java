package com.wistron.swpc.commonlib.listviewdemo;

/**
 * Data For Demo
 * @author Mumu
 *
 */
public class ListViewData {

	public static final String[] DATA = { "AbbaylocAbbaye de Belloc", "Abbaye de BellocAbbaye du Mont des Cats",
		"Abbaye de BellocAbertam", "鼠牛虎兔龙蛇马羊猴鸡狗猪", "Abbaye de BellocAckawi",
		"Abbaye de BellocAcorn", "Abbaye de BellocAdelost", "Abbaye de BellocAffidelice au Chablis",
		"AfueasdfAbbaye de Belloc", "Abbaye dadsf BellocAirag", "Abbaye de BellocAiredale",
		"Abbaye de BellocAisy Cendre", "Abbaye de BellocAllgauer Emmentaler", "Abbaye de BellocAlverca",
		"Abbaye de BellocAmbert", "Abbaye de BellocAmerican Cheese", "Ami duasdfelloc",
		"Abbaye de BellocAnejo Enchilado", "3.14159265354", "快使用双节棍哼哼哈嘿!快使用双节棍哼哼哈嘿!", "鼠牛虎兔", "龙蛇马羊", "猴鸡狗猪", "金木水火土", "甲乙丙丁",
		"午己庚辛壬葵", "iphoneiphoneiphoneiphoneiphoneiphoneiphoneiphoneiphoneiphoneiphone", "appleappleappleappleappleappleappleappleappleapple", "iPadiPadiPadiPadiPadiPadiPadiPadiPadiPadiPadiPadiPadiPad", 
		"SamsungSamsungSamsungSamsungSamsungSamsungSamsung", "MIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUIMIUI",
		"GalaxyGalaxyGalaxyGalaxyGalaxy", "星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢星矢", 
		"李白歌手我是洗衣液李白歌手我是洗衣液李白歌手我是洗衣液", 
		"紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙紫龙", "沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加沙加", "你竟然看完了,能读到这里的少年,祝你前途什锦!" };
	public static final String[] DATA_TITLE = { "Abbaye de Belloc", "Abbaye du Mont des Cats", "Abertam", "Abondance",
		"Ackawi", "Acorn", "Adelost", "Affidelice au Chablis", "Afuega'l Pitu", "Airag", "Airedale", "Aisy Cendre",
		"Allgauer Emmentaler", "Alverca", "Ambert", "American Cheese", "Ami du Chambertin", "Anejo Enchilado",
		"Anneau du Vic-Bilh", "Anthoriro", "Appenzell", "Aragon", "Ardi Gasna", "Ardrahan", "Armenian String",
		"Aromes au Gene de Marc", "Asadero", "Asiago", "Woodside Cabecou", "Xanadu", "Xynotyro", "Yarg Cornish",
		"Yarra Valley Pyramid", "Yorkshire Blue", "Zamorano", "Zanetti Grana Padano", "Zanetti Parmigiano Reggiano" };
}
